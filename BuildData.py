...
NON_HELPFUL_COLUMNS = ['trtid10','trtid10num','cntyid10','changetype_1pct','changetype_5pct',
    ...
    #The NON_HELPFUL_COLUMNS are simply columns that are used to classify tracts, they won't contribute
    #anything to the neural network (or they will, but they're not variables that can be seen in real life)

df = pd.read_csv('Datasets/LTDB_DP.csv')
df_flawed_gentrified = pd.read_csv('Datasets/census_tracts.csv') ##Read CSV datasets
...
df_total['gentrif'] = df_total['gentrif'].astype(int) #Notice that our binary classification of the gentrif
#variable is stored as a float, this is not necessary so let's keep it stored as an integer, 0 or 1

columns = df_total.columns
columns_to_scale = columns[:len(columns)-1] #Don't want to scale the gentrif column
scaled_variables = df_total.copy() #We will standardize the non-gentrif data in a copy of our original dataframe
features = scaled_variables[columns_to_scale] #The scaler object will be applied to this
...
features = scaler.fit_transform(features.values)  #Give each data column a mean of 0 and variance of 1 so the
#distribution of data in each column doesn't inadvertently give any one variable more weight than another
scaled_variables[columns_to_scale] = features

print("What does this processed data look like now?")
print(scaled_variables.describe())
print("====================================================")

print("Storing processed data as a csv file called 'BuildData_Set.csv'")
scaled_variables.to_csv('Datasets/BuildData_Set.csv', index=False)
